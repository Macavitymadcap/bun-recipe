...
import { CreateRecipeData, RecipeService } from "../database/services/recipe-service";
import { Container } from "./container";
import { BaseRoute } from "./base-route";
import { Alert, type AlertProps } from "../components/Alert";
import { GetRecipeByIDResponse } from "../components/GetRecipeByIdResponse";
import { ReadRecipe, ReadRecipeProps } from "../components/ReadRecipe";
import { CreateRecipeResponse } from "../components/CreateRecipeResponse";
...
export class RecipeRoute extends BaseRoute {
  ...
  private async createRecipe(context: Context): Promise<Response> {
    ...
    try {
      ...
      if (!formData.name || !formData.servings || 
          !formData.ingredients?.length || !formData.method?.length) {
        alert = {
          alertType: "danger",
          title: "Validation Error",
          message: "Name, servings, at least one ingredient, and at least one method step are required.",
        };

        return context.html(
          CreateRecipeResponse({ alert, recipe: undefined }),
        );
      }
  ...
  private async getAllRecipes(context: Context): Promise<Response> {
    ...
    try {
      ...
      return context.html(`${recipes.map((recipe: ReadRecipeProps) => ReadRecipe(recipe)).join("")}`);
    } catch (error) {
  ...
  private async parseRecipeFormData(context: Context): Promise<CreateRecipeData> {
    ...
    const calories_per_portion = formData.get("calories_per_portion") ? 
      parseInt(formData.get("calories_per_portion") as string) : undefined;
    const preparation_time = formData.get("preparation_time") as string || undefined;
    ...
    while (formData.has(`ingredients[${ingredientIndex}][name]`)) {
      const quantity = parseFloat(formData.get(`ingredients[${ingredientIndex}][quantity]`) as string);
      const unit = formData.get(`ingredients[${ingredientIndex}][unit]`) as string || undefined;
      const ingredientName = formData.get(`ingredients[${ingredientIndex}][name]`) as string;
    ...
    while (formData.has(`method[${methodIndex}][instruction]`)) {
      const instruction = formData.get(`method[${methodIndex}][instruction]`) as string;
