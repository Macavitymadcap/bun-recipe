...
P.inverseTangent = P.atan = function () {
  ...
  for (; i !== -1; ) {
    ...
    px = px.times(x2);
    r = t.plus(px.div((n += 2)));
    if (r.d[j] !== undefined) for (i = j; r.d[i] === t.d[i] && i--; );
  }
...
function random(sd) {
  var d,
    ...
    r = new this(1),
    rd = [];
  if (sd === undefined) sd = this.precision;
...
// src/conversion/temperature-converter.ts
...
// src/conversion/mass-converter.ts
class MassConverter {
...
// src/conversion/volume-converter.ts
...
var convert = (value, fromUnit, toUnit) => {
  let result;
  if (isMassUnit(fromUnit) && isMassUnit(toUnit)) {
  ...
  const rounded = new decimal_default(result).toDecimalPlaces(2);
